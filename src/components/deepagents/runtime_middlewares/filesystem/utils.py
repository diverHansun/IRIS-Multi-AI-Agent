...
def assert_path_allowed(path: str, security: FilesystemSecurity) -> None:
    """Validate the path against allow/deny lists and file extension rules."""
    normalized = normalize_virtual_path(path)

    if security.allowed_paths:
        allowed = any(normalized.startswith(normalize_virtual_path(prefix)) for prefix in security.allowed_paths)
        if not allowed:
            raise ValueError(f"Path '{path}' is outside the permitted prefixes")

    if security.excluded_paths:
        if any(normalized.startswith(normalize_virtual_path(prefix)) for prefix in security.excluded_paths):
            raise ValueError(f"Path '{path}' is explicitly blocked")

    _, extension = os.path.splitext(normalized)
    if security.excluded_extensions and extension in security.excluded_extensions:
        raise ValueError(f"Files with extension '{extension}' are not allowed")


def enforce_limits(content: str, *, security: FilesystemSecurity) -> None:
    """Raise an error if the content exceeds configured limits."""
    if len(content.encode("utf-8")) > security.max_file_size:
        raise ValueError("Content exceeds configured file size limit")

    if len(content.splitlines()) > security.max_file_lines:
        raise ValueError("Content exceeds configured line count limit")
...
def string_to_file_data(content: str, *, security: FilesystemSecurity) -> FileData:
    """Create FileData payload from raw string content."""
    enforce_limits(content, security=security)
    timestamp = iso_now()
    return FileData(content=content.splitlines(), created_at=timestamp, modified_at=timestamp)


def update_file_data(existing: FileData, new_content: str, *, security: FilesystemSecurity) -> FileData:
    """Return a copy of FileData with updated content and timestamp."""
    enforce_limits(new_content, security=security)
    updated = FileData(
